...
var GenerateSignature = function (payload) {
    return jsonwebtoken_1.default.sign(payload, config_1.APP_SECRET, { expiresIn: "30m" });
};
exports.GenerateSignature = GenerateSignature;
var ValidateSignature = function (req) {
    // const signature = req.headers.authorization || req.headers.Authorization;
    var signature = req.get("Authorization");
    if (signature) {
        try {
            var payload = jsonwebtoken_1.default.verify(signature.split(" ")[1], config_1.APP_SECRET);
            console.log(payload);
            req.user = payload;
            return true;
        }
